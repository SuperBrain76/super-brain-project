"use client";

// Phase 1 — Target Lock
// Tap green targets, avoid red decoys.
// Circles spawn every ~600ms and drift across the screen.

import { useCallback, useEffect, useRef, useState } from "react";
import { Sounds } from "@/lib/sounds";
import type { PhaseResult } from "@/lib/focus-test";

interface Props {
  soundEnabled: boolean;
  onComplete:   (result: PhaseResult) => void;
}

const DURATION_MS  = 45_000;
const CIRCLE_SIZE  = 60;
const LIFETIME_MS  = 1600; // circles vanish after 1.6s — creates constant urgency

const DRIFT_DIRS = [
  "drift-ne","drift-e","drift-se","drift-s",
  "drift-sw","drift-w","drift-nw","drift-n",
] as const;

interface Circle {
  id:      number;
  type:    "target" | "decoy";
  x:       number;   // % left
  y:       number;   // % top
  drift:   string;
  spawnMs: number;   // ms since phase start
}

let _nextId = 0;

function makeCircle(elapsed: number): Circle {
  const m = 10; // margin % so circles don't spawn at edges
  return {
    id:      _nextId++,
    type:    Math.random() < 0.62 ? "target" : "decoy",
    x:       m + Math.random() * (100 - m * 2),
    y:       m + Math.random() * (100 - m * 2),
    drift:   DRIFT_DIRS[Math.floor(Math.random() * DRIFT_DIRS.length)],
    spawnMs: elapsed,
  };
}

export default function TargetLock({ soundEnabled, onComplete }: Props) {
  const [circles,   setCircles]   = useState<Circle[]>([]);
  const [timeLeft,  setTimeLeft]  = useState(DURATION_MS);
  const [score,     setScore]     = useState(0);
  const [streak,    setStreak]    = useState(0);
  const [streakMax, setStreakMax] = useState(0);
  const [feedback,  setFeedback]  = useState<{ label: string; color: string } | null>(null);

  const startRef     = useRef(Date.now());
  const hitsRef      = useRef(0);
  const missesRef    = useRef(0);
  const reactionsRef = useRef<number[]>([]);
  const completedRef = useRef(false);
  const streakRef    = useRef(0);
  const streakMaxRef = useRef(0);
  const scoreRef     = useRef(0);

  const finish = useCallback(() => {
    const total    = hitsRef.current + missesRef.current;
    const accuracy = total > 0 ? hitsRef.current / total : 0;
    const avgMs    = reactionsRef.current.length
      ? reactionsRef.current.reduce((s, v) => s + v, 0) / reactionsRef.current.length
      : LIFETIME_MS;
    const speedBonus  = Math.max(0, (900 - avgMs) / 9);
    const phaseScore  = Math.min(100, Math.round(accuracy * 72 + speedBonus * 0.28));
    if (soundEnabled) Sounds.phaseComplete();
    onComplete({
      phase: 1, score: phaseScore, accuracy,
      avgReactionMs: Math.round(avgMs), streakMax: streakMaxRef.current,
      extras: { hits: hitsRef.current, misses: missesRef.current },
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [soundEnabled, onComplete]);

  // Countdown + circle expiry
  useEffect(() => {
    const interval = setInterval(() => {
      const elapsed = Date.now() - startRef.current;
      const left    = Math.max(0, DURATION_MS - elapsed);
      setTimeLeft(left);
      setCircles((prev) => prev.filter((c) => elapsed - c.spawnMs < LIFETIME_MS));
      if (left === 0 && !completedRef.current) {
        completedRef.current = true;
        finish();
      }
    }, 60);
    return () => clearInterval(interval);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Spawn circles — guaranteed cadence, ramps up over time
  useEffect(() => {
    // Seed the field immediately so the player isn't staring at nothing
    const elapsed0 = Date.now() - startRef.current;
    setCircles([makeCircle(elapsed0), makeCircle(elapsed0), makeCircle(elapsed0)]);

    const spawn = () => {
      if (completedRef.current) return;
      const elapsed  = Date.now() - startRef.current;
      const progress = elapsed / DURATION_MS;
      // 580ms at start → 340ms at end
      const nextDelay = Math.round(580 - progress * 240);

      setCircles((prev) => {
        if (prev.length >= 10) return prev; // hard cap
        return [...prev, makeCircle(elapsed)];
      });

      setTimeout(spawn, nextDelay);
    };

    const t = setTimeout(spawn, 380);
    return () => clearTimeout(t);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleClick = (circle: Circle, e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    if (completedRef.current) return;

    const elapsed    = Date.now() - startRef.current;
    const reactionMs = elapsed - circle.spawnMs;

    setCircles((prev) => prev.filter((c) => c.id !== circle.id));

    if (circle.type === "target") {
      hitsRef.current++;
      reactionsRef.current.push(reactionMs);
      const ns = streakRef.current + 1;
      streakRef.current = ns;
      if (ns > streakMaxRef.current) streakMaxRef.current = ns;
      setStreak(ns);
      setStreakMax(streakMaxRef.current);

      const mult   = ns >= 15 ? 2.5 : ns >= 7 ? 2 : ns >= 3 ? 1.5 : 1;
      const points = Math.round(10 * mult);
      scoreRef.current += points;
      setScore(scoreRef.current);
      setFeedback({ label: `+${points}${mult > 1 ? ` ×${mult}` : ""}`, color: "#00e676" });
      setTimeout(() => setFeedback(null), 550);

      if (soundEnabled) {
        if (ns === 3 || ns === 7 || ns === 15) Sounds.streak();
        else Sounds.hit();
      }
    } else {
      missesRef.current++;
      streakRef.current = 0;
      setStreak(0);
      scoreRef.current = Math.max(0, scoreRef.current - 5);
      setScore(scoreRef.current);
      setFeedback({ label: "−5", color: "#ff3d00" });
      setTimeout(() => setFeedback(null), 550);
      if (soundEnabled) Sounds.miss();
    }
  };

  const progress    = 1 - timeLeft / DURATION_MS;
  const secsLeft    = Math.ceil(timeLeft / 1000);
  const streakColor = streak >= 15 ? "#ff6d00" : streak >= 7 ? "#ffab00" : streak >= 3 ? "#00d4ff" : "#64748b";

  return (
    <div className="flex flex-col gap-3 w-full h-full select-none">
      {/* HUD */}
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          <span className="text-white text-lg font-extrabold number-display tabular-nums">{score}</span>
          {streak >= 2 && (
            <span className="text-xs font-bold px-2 py-0.5 rounded-sm border"
              style={{ color: streakColor, borderColor: `${streakColor}40`, background: `${streakColor}15` }}>
              ×{streak >= 15 ? "2.5" : streak >= 7 ? "2.0" : "1.5"} STREAK
            </span>
          )}
        </div>
        <span className="text-cockpit-muted text-sm font-mono">{secsLeft}s</span>
      </div>

      {/* Timer bar */}
      <div className="h-0.5 w-full bg-cockpit-border rounded-full overflow-hidden">
        <div className="h-full bg-[#00d4ff]"
          style={{ width: `${(1 - progress) * 100}%`, transition: "width 0.06s linear" }} />
      </div>

      {/* Play area */}
      <div
        className="relative flex-1 bg-cockpit-surface border border-cockpit-border rounded-sm overflow-hidden"
        style={{ minHeight: 300 }}
      >
        {circles.map((c) => {
          const elapsed   = Date.now() - startRef.current - c.spawnMs;
          const fadeStart = LIFETIME_MS - 350;
          const opacity   = elapsed > fadeStart ? Math.max(0, (LIFETIME_MS - elapsed) / 350) : 1;
          const color     = c.type === "target" ? "#00e676" : "#ff3d00";
          return (
            <button
              key={c.id}
              onClick={(e) => handleClick(c, e)}
              onTouchStart={(e) => handleClick(c, e)}
              className="absolute focus:outline-none active:scale-90 transition-transform duration-75"
              style={{
                left:        `${c.x}%`,
                top:         `${c.y}%`,
                width:       CIRCLE_SIZE,
                height:      CIRCLE_SIZE,
                borderRadius: "50%",
                border:      `2px solid ${color}`,
                background:  `${color}20`,
                boxShadow:   `0 0 16px ${color}50`,
                opacity,
                animation:   `${c.drift} ${LIFETIME_MS}ms linear forwards`,
                touchAction: "manipulation",
              }}
            />
          );
        })}

        {/* Score feedback — centered */}
        {feedback && (
          <div
            className="absolute pointer-events-none text-sm font-extrabold number-display"
            style={{
              color: feedback.color,
              left: "50%", top: "50%",
              transform: "translate(-50%,-50%)",
              animation: "moduleEnter 0.55s ease-out forwards",
              textShadow: `0 0 10px ${feedback.color}`,
            }}
          >
            {feedback.label}
          </div>
        )}
      </div>

      {/* Legend */}
      <div className="flex items-center justify-center gap-6">
        <div className="flex items-center gap-2">
          <div className="w-3.5 h-3.5 rounded-full border-2 border-[#00e676] bg-[#00e67615]" />
          <span className="text-cockpit-muted text-xs">TAP</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3.5 h-3.5 rounded-full border-2 border-[#ff3d00] bg-[#ff3d0015]" />
          <span className="text-cockpit-muted text-xs">AVOID</span>
        </div>
      </div>
    </div>
  );
}
