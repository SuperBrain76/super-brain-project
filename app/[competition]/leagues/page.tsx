...
function LeaguesContent() {
  ...
  useEffect(() => {
    async function load() {
      // Resolve the competition FIRST so the league query is scoped to the
      // same one. These used to run in parallel against a shared "wc2026"
      // literal; with several competitions live, a parallel call would have
      // to guess the slug and could list another competition's leagues.
      const compResult = await resolveCompetition(competitionSlug);
      const { competition: comp, error: compError } = compResult;
      const leaguesResult = user && comp
        ? await getMyLeaguesBySlug(comp.slug)
        : { leagues: [] as PredictionLeague[], error: null };
    ...
    if (!authLoading) load();
  }, [user, authLoading, competitionSlug]);
  ...
  const handleJoin = useCallback(async () => {
    ...
    if (myLeagues.some((l) => l.id === foundLeague.id)) { router.push(`/${competitionSlug}/leagues/${foundLeague.id}`); return; }
